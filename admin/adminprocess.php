...
class AdminProcess
{
   ...
   function procUpdateLevel(){
      ...
      /* Update user level */
         ...
         $database->updateUserLevel($subuser, (int)$_POST['updlevel']);
         header("Location: ".$session->referrer);
      }
   }
   ...
   function procDeleteUser(){
      ...
      /* Delete user from database */
         ...
         $stmt = $database->prepare("DELETE FROM ".TBL_USERS." WHERE username = ?");
		 $stmt->bind_param("s",$subuser);
         $stmt->execute();
         header("Location: ".$session->referrer);
      }
   }
   ...
   function procDeleteInactive(){
	  ...
	  $query = "DELETE FROM ".TBL_USERS." WHERE timestamp < ? AND userlevel != ".ADMIN_LEVEL;
	  $stmt = $database->prepare($query);
	  $stmt->bind_param("i",$inact_time);
      $stmt->execute();
      header("Location: ".$session->referrer);
   }
   ...
   function checkUsername($uname, $ban = false){
      ...
      if(!$subuser || strlen($subuser = trim($subuser)) == 0){
         ...
         if(strlen($subuser) < 5 || strlen($subuser) > 30 || !ctype_alnum($subuser) || (!$ban && !$database->usernameTaken($subuser))) {
